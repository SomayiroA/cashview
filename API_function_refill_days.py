...
async def train_model(
    ...
    try:
        ...
        df2 = df[[amount_col, balance_col, "date", "day", "dayofweek", "month", "year"]].groupby(
        ...
        X = df4[["month", "Month_end", "is_weekend", "is_holiday", "dayofweek", "year"]]
        y = df4[amount_col]
...
async def predict(
    ...
    try:
        ...
        for day in range(len(pred_df)):
            daily_pred = float(trained_model.predict(pred_df.iloc[[day]][["month", "Month_end", "is_weekend", "is_holiday", "dayofweek", "year"]])[0])
            breakdown.append({
                "day_number": day + 1,
