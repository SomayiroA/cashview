...
# Global variables to store trained model and metadata
trained_model = None
model_metadata = {
    "avg_refill": None,
    "column_names": None,
...
async def train_model(
    ...
    try:
        ...
        X = df3[["month", "Month_end", "is_weekend", "is_holiday"]]
        y = df3[amount_col]
...
async def predict(
    ...
    try:
        ...
        for day in range(len(pred_df)):
            daily_pred = float(trained_model.predict(pred_df.iloc[[day]])[0])
            breakdown.append({
                "day_number": day + 1,
